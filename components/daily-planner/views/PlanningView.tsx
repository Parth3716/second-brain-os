...
import type { Task, Habit, DailyPlanItem } from "@/types/daily-planner";
...
export default function PlanningView({
  ...
  const handleForceHabit = async (id: string) => {
    ...
    const result = await addHabitToQueue(id);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="space-y-3">
              <button
                ...
                className="flex items-center gap-3 text-slate-300 hover:text-white transition-colors w-full group outline-none"
              >
                <ChevronDown
                  className={`w-4 h-4 text-slate-500 transition-transform duration-300 ${isHabitsOpen ? "" : "-rotate-90"}`}
                />
              ...
              <AnimatePresence initial={false}>
                {isHabitsOpen && (
                  ...
                  >
                    {routines.length === 0 ? (
                      <li className="text-xs text-slate-600 italic py-2 px-6">
                      ...
                      routines.map((habit) => (
                        <li
                          key={habit.id}
                          className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-2 pl-4 text-sm text-slate-300 flex items-center justify-between group/item transition-colors hover:bg-white/[0.04]"
                        >
                          ...
                          <button
                            onClick={() => handleForceHabit(habit.id)}
                            disabled={loadingItems.includes(habit.id)}
                            className="flex-shrink-0 text-slate-400 hover:text-white bg-white/[0.05] hover:bg-emerald-500/20 p-2 rounded-lg transition-all border border-white/[0.05]"
                          >
                            {loadingItems.includes(habit.id) ? (
            ...
            <div className="space-y-3">
              ...
              >
                <ChevronDown
                  className={`w-4 h-4 text-slate-500 transition-transform duration-300 ${isTasksOpen ? "" : "-rotate-90"}`}
                />
                      ...
                      {backlogTasks.length === 0 ? (
                        ...
                        backlogTasks.map((task) => (
                          ...
                          >
                            ...
                            <form
                              action={(formData) =>
                                handleForceTask(task.id, formData)
                              }
                            ...
                            >
                              ...
                              <button
                                type="submit"
                                disabled={loadingItems.includes(task.id)}
                                className="text-slate-400 hover:text-white bg-white/[0.05] hover:bg-indigo-500/20 p-2 rounded-lg transition-all disabled:opacity-50 border border-white/[0.05]"
                              >
                                {loadingItems.includes(task.id) ? (
                                  <Loader2 className="w-4 h-4 animate-spin" />
                                ) : (
                                  <Plus className="w-4 h-4" />
                                )}
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="flex justify-between items-end border-b border-white/10 pb-3">
              <h2 className="text-sm md:text-base font-bold text-slate-300 uppercase tracking-widest">
            ...
            <div className="min-h-[300px] md:min-h-[500px] flex flex-col">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
                ...
                >
                  <button
                    type="submit"
                    disabled={todaysQueue.length === 0 || isStarting}
                  ...
                  >
                    {isStarting ? (
                      ...
                      <>
                        <Play className="w-4 h-4 fill-current" /> BEGIN WORKDAY
                      </>
                    )}
                  </button>
                </form>
                ...
                >
                  <button
                    type="submit"
                    disabled={isResting}
                    className="w-full bg-white/[0.03] hover:bg-red-500/10 text-slate-300 hover:text-red-400 border border-white/5 hover:border-red-500/30 py-4 rounded-2xl font-bold tracking-widest text-xs md:text-sm transition-all flex justify-center items-center gap-2"
                  >
                    {isResting ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
                ...
                <Link
                  ...
                  className="sm:col-span-1 bg-white/[0.03] hover:bg-white/[0.08] text-slate-300 hover:text-white border border-white/5 py-4 rounded-2xl font-bold tracking-widest text-xs md:text-sm transition-all flex justify-center items-center gap-2"
                >
                  ...
                  {todaysQueue.length === 0 ? (
                    ...
                    >
                      ...
                      <p className="text-slate-500 text-xs md:text-sm mt-1">
                        Click the + buttons on the left to draft your day.
                      </p>
                    </motion.div>
                  ) : (
                    todaysQueue.map((item, index: number) => (
                      <motion.div
                        layout

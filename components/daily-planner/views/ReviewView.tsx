...
import type { DailyRecord, TimeEntry } from "@/types/daily-planner";
...
export default function ReviewView({
  ...
  const completedItems = dailyRecord.items.filter(
  ...
  const liveSessions = timeEntries.filter(
    (e) => e.entryType === "LIVE_TRACKED",
  ).length;
  const manualSessions = timeEntries.filter(
    (e) => e.entryType === "MANUAL",
  ).length;

  const liveSeconds = timeEntries
    .filter((e) => e.entryType === "LIVE_TRACKED")
    .reduce((sum, e) => sum + (e.durationSeconds || 0), 0);
  const manualSeconds = timeEntries
    .filter((e) => e.entryType === "MANUAL")
    .reduce((sum, e) => sum + (e.durationSeconds || 0), 0);

  const livePercent = totalSeconds > 0 ? (liveSeconds / totalSeconds) * 100 : 0;
  const manualPercent =
    totalSeconds > 0 ? (manualSeconds / totalSeconds) * 100 : 0;
    ...
    <main className="min-h-screen bg-[#030712] relative flex justify-center p-4 md:p-12 overflow-hidden selection:bg-indigo-500/30">
      ...
      <div className="w-full max-w-[90rem] relative z-10 px-0 sm:px-4 xl:px-10 flex flex-col space-y-10">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/5 pb-6 gap-4">
          ...
          <div
            className={`flex items-center gap-3 border px-4 py-2 rounded-full shadow-inner ${pillColor}`}
          >
            <div
              className={`w-2 h-2 rounded-full animate-pulse ${isPartial ? "bg-amber-400" : "bg-emerald-400"}`}
            />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          ...
          >
            ...
            <motion.div
              variants={itemVars}
              className="bg-[#0c1222] border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col items-center text-center relative overflow-hidden"
            >
              ...
              <p className="text-slate-400 text-sm leading-relaxed mb-8">
                ...
                <strong className="text-slate-200">
                  {completedItems} out of {totalItems}
                </strong>{" "}
                scheduled items. Your execution metrics have been synced.
              </p>

              {/* Action Buttons Side-by-Side */}
              <div className="grid grid-cols-2 w-full gap-3">
                <Link
                  href="/daily-planner/manage"
                  className="py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all font-bold text-xs md:text-sm flex flex-col items-center justify-center gap-1 shadow-lg shadow-indigo-900/20"
                >
                  <Settings2 className="w-5 h-5" /> Manage Tomorrow
                </Link>
                <Link
                  href="/daily-planner/analytics"
                  className="py-4 bg-white/[0.03] hover:bg-white/[0.08] border border-white/10 text-slate-300 hover:text-white rounded-xl transition-all font-bold text-xs md:text-sm flex flex-col items-center justify-center gap-1"
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <motion.div
                variants={itemVars}
                className="bg-white/[0.02] border border-white/[0.05] p-5 rounded-2xl flex flex-col gap-2"
              >
              ...
              <motion.div
                variants={itemVars}
                className="bg-white/[0.02] border border-white/[0.05] p-5 rounded-2xl flex flex-col justify-center"
              >
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between text-xs font-medium text-slate-400">
                    <span className="flex items-center gap-1">
                      <PlayCircle className="w-3 h-3 text-emerald-400" /> Live
                    </span>{" "}
                    <span className="text-slate-200">{liveSessions}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs font-medium text-slate-400">
                    <span className="flex items-center gap-1">
                      <History className="w-3 h-3 text-blue-400" /> Manual
                    </span>{" "}
                    <span className="text-slate-200">{manualSessions}</span>
                  </div>
                </div>
              </motion.div>
            </div>
            ...
            {timeEntries.length === 0 ? (
              ...
              <div className="relative border-l-2 border-white/10 ml-4 md:ml-6 pb-10 space-y-8 mt-4">
                {timeEntries.map((entry) => {
                  ...
                  const mins = entry.durationSeconds
                    ? Math.ceil(entry.durationSeconds / 60)
                    : 0;
                  ...
                  return (
                    <motion.div
                      variants={itemVars}
                      key={entry.id}
                      className="relative pl-6 md:pl-10 group/card"
                    >
                      {/* Timeline Dot */}
                      <div
                        className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-4 border-[#030712] ${isLive ? "bg-emerald-500" : "bg-blue-500"} group-hover/card:scale-125 transition-transform duration-300`}
                      />

                      {/* Time Label */}
                      <span className="block text-[10px] md:text-xs font-bold text-slate-500 mb-2 font-mono tracking-wider">
                        {formatTimeHM(entry.startedAt)}{" "}
                        {entry.endedAt && `- ${formatTimeHM(entry.endedAt)}`}
                      </span>
                      ...
                      <div className="bg-white/[0.02] hover:bg-[#0c1222] border border-white/[0.05] rounded-2xl p-4 md:p-5 transition-colors shadow-lg flex justify-between items-start gap-4">
                        <div className="flex flex-col gap-1 min-w-0">
                          <span className="font-semibold text-slate-200 text-base md:text-lg leading-tight truncate">
                            {entry.title}
                          </span>
                          <div className="flex items-center gap-3 mt-1">
                            <span
                              className={`text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 ${isLive ? "text-emerald-400" : "text-blue-400"}`}
                            >
                              {isLive ? (
        ...
        {editingEntry && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-[#0c1222] border border-white/10 rounded-3xl shadow-2xl w-full max-w-[95vw] md:max-w-md overflow-hidden"
            >
              <div className="flex justify-between items-center bg-white/[0.02] p-4 md:p-5 border-b border-white/5">
                <h3 className="font-bold text-white flex items-center gap-2 text-sm md:text-base">
                  <Edit2 className="w-4 h-4 md:w-5 md:h-5 text-blue-400" /> Edit
                  Timeline Entry
                </h3>
                <button
              ...
              <form
                action={async (fd: FormData) => {
                  const result = await updateTimeEntry(editingEntry!.id, fd);
                  if (!result.success) {
              ...
              >
                <div className="p-5 md:p-6 space-y-5 md:space-y-6">
                  <div>
                    ...
                    <div className="bg-black/20 border border-white/5 rounded-xl p-3 md:p-3.5 text-slate-300 text-xs md:text-sm font-medium truncate">
                      {editingEntry.title}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                    <div>
                      ...
                      <input
                        ...
                        name="startTime"
                        required
                        defaultValue={getInputTime(editingEntry.startedAt)}
                        className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-3.5 text-slate-200 focus:border-blue-500 focus:outline-none transition-all text-sm md:text-base [color-scheme:dark]"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
                      ...
                      <input
                        ...
                        defaultValue={
                          editingEntry.endedAt
                            ? getInputTime(editingEntry.endedAt)
                            : ""
                        }
                        className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-3.5 text-slate-200 focus:border-blue-500 focus:outline-none transition-all text-sm md:text-base [color-scheme:dark]"
                      />
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-3.5 text-slate-200 focus:border-blue-500 focus:outline-none min-h-[80px] md:min-h-[100px] transition-all text-sm md:text-base"
                    />
                  </div>
                </div>
                <div className="flex justify-end gap-3 p-4 md:p-5 border-t border-white/5 bg-white/[0.01]">
                  <button
                    type="button"
                    onClick={() => setEditingEntry(null)}
                    className="px-4 md:px-5 py-2 md:py-2.5 text-xs md:text-sm font-medium text-slate-400 hover:text-white transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-5 md:px-6 py-2 md:py-2.5 text-xs md:text-sm font-bold bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-colors shadow-lg shadow-blue-900/20 flex items-center gap-2"
                  >
        ...
        {isAddingAdhoc && (
          ...
          >
            ...
            >
              <div className="flex justify-between items-center bg-white/[0.02] p-4 md:p-5 border-b border-white/5">
                <h3 className="font-bold text-white flex items-center gap-2 text-sm md:text-base">
                  <Plus className="w-4 h-4 md:w-5 md:h-5 text-indigo-400" /> Add
                  Past Activity
                </h3>
                <button
              ...
              <form
                action={async (fd: FormData) => {
                  const result = await addAdhocLog(fd);
                  if (!result.success) {
                    showToast(result.error);
                    return;
                  }
                ...
                <div className="p-5 md:p-6 space-y-5 md:space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      name="title"
                      required
                      placeholder="e.g. Spontaneous Meeting"
                      className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-3.5 text-slate-200 focus:border-indigo-500 focus:outline-none transition-all text-sm md:text-base"
                    />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                    <div>
                      ...
                      <input
                        ...
                        required
                        defaultValue="14:00"
                        className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-3.5 text-slate-200 focus:border-indigo-500 focus:outline-none transition-all text-sm md:text-base [color-scheme:dark]"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-black/20 border border-white/10 rounded-xl p-3 md:p-3.5 text-slate-200 focus:border-indigo-500 focus:outline-none min-h-[80px] md:min-h-[100px] transition-all text-sm md:text-base"
                    />
                  </div>
                </div>
                <div className="flex justify-end gap-3 p-4 md:p-5 border-t border-white/5 bg-white/[0.01]">
                  ...
                  <button
                    type="submit"
                    className="px-5 md:px-6 py-2 md:py-2.5 text-xs md:text-sm font-bold bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-colors shadow-lg shadow-indigo-900/20 flex items-center gap-2"
                  >

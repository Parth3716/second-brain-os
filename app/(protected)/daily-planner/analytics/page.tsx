...
export default async function AnalyticsPage() {
  ...
  const livePercent = totalSeconds > 0 ? Math.round((liveSeconds / totalSeconds) * 100) : 0;

  const adhocSeconds = timeEntries.filter(e => e.planItemId === null).reduce((sum, e) => sum + (e.durationSeconds || 0), 0);
  const adhocPercent = totalSeconds > 0 ? Math.round((adhocSeconds / totalSeconds) * 100) : 0;
  ...
  planItems.filter(i => i.status === "COMPLETED" && i.originalCycles).forEach(item => {
    // 1 cycle = 45 mins = 2700 seconds (Adjust if your cycles are different)
    totalEstimatedSeconds += (item.originalCycles! * 45 * 60); 
    const actualTime = timeEntries.filter(e => e.planItemId === item.id).reduce((sum, e) => sum + (e.durationSeconds || 0), 0);
    totalActualSeconds += actualTime;
  });

  let estimationAccuracy = 0;
  if (totalEstimatedSeconds > 0 && totalActualSeconds > 0) {
    estimationAccuracy = Math.round((Math.min(totalEstimatedSeconds, totalActualSeconds) / Math.max(totalEstimatedSeconds, totalActualSeconds)) * 100);
  }
